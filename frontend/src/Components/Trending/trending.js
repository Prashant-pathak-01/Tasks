...
import dummyData from '../Discussion/dummyData'; // Use relative path instead
...
function TrendingPosts() {
  ...
  useEffect(() => {
    const sortedPosts = dummyData
      .map((post) => ({
        ...post,
        score: post.likes * 2 + post.shared * 3 + post.replies * 1,
      }))
      .sort((a, b) => b.score - a.score);

    setTrendingPosts(sortedPosts.slice(0, 6));
  }, []);
